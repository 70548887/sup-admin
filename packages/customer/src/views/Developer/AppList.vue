...
async function fetchList() {
  loading.value = true
  try {
    const res: any = await customerClient.get('/openapi/customer/App/List', {
...
async function handleRotate(row: AppItem) {
  try {
    ...
    const res: any = await customerClient.post('/openapi/customer/App/RotateSecret', {
      app_id: row.app_id,
    })
...
async function handleToggle(row: AppItem) {
  ...
  try {
    await customerClient.post(`/openapi/customer/App/${action}`, { app_id: row.app_id })
...
async function handleDelete(row: AppItem) {
  try {
    ...
    await customerClient.post('/openapi/customer/App/Delete', { app_id: row.app_id })
