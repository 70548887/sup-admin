<template>
  <div class="page-container">
    <PageHeader title="导入卡密" subtitle="上传或粘贴卡密内容，导入到您的商品库存">
      <template #actions>
    ...
    <el-card shadow="never">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="选择商品" prop="goodsId">
          <el-select v-model="form.goodsId" placeholder="请选择您的商品" filterable style="width: 100%;">
            <el-option v-for="g in goodsOptions" :key="g.id" :label="g.name" :value="g.id" />
          </el-select>
        </el-form-item>

        <el-form-item label="批次名称" prop="batchName">
          <el-input v-model="form.batchName" placeholder="请输入批次名称" maxlength="50" show-word-limit />
        </el-form-item>
        ...
        <el-form-item v-if="importMode === 'file'" label="卡密文件">
          <el-upload
            :auto-upload="false"
            :limit="1"
            accept=".txt"
        ...
        <el-form-item v-if="importMode === 'paste'" label="卡密内容">
          <el-input
...
function updatePreviewCount() {
  previewCount.value = form.content.split('\n').filter((l) => l.trim()).length
}
...
async function handleSubmit() {
  ...
  try {
    ...
    const res = await apiClient.post('/supplier/cards/import-async', {
...
function startPolling() {
  pollTimer = setInterval(async () => {
    try {
      const data = await apiClient.get(`/supplier/cards/import-task/${importTaskId.value}`) as any
...
async function loadGoodsOptions() {
  try {
    const data = await apiClient.get('/supplier/goods/options') as any
